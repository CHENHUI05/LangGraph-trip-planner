...
class MapSearchInput(BaseModel):
    keywords: str = Field(description="搜索关键词，例如：北京故宫、五星级酒店、特色餐厅")
...
class Hotel(BaseModel):
    ...
    # 👇 强化这里的 description，禁止大模型输出英文
    type: str = Field(default="", description="住宿类型，必须使用纯中文，例如：'经济型酒店'、'豪华酒店'、'特色民宿'，绝不能输出英文单词！")
    estimated_cost: int = Field(default=0, description="预估费用(元/晚)")


class DayPlan(BaseModel):
    """单日行程"""
    # 👇 强化这里的 description，强制要求日期在指定范围内
    date: str = Field(..., description="当前行程的日期，必须是 YYYY-MM-DD 格式。注意：必须严格按照用户请求的开始和结束日期排期，不能使用历史或伪造日期！")
    day_index: int = Field(..., description="第几天(从0开始)")
    description: str = Field(..., description="当日行程概述，不要太长")
