...
import { useRouter, useRoute } from 'vue-router' // 👈 增加了 useRoute
...
const route = useRoute() // 👈 实例化 route，用来获取网址参数
...
// 💡 提取一个公共的初始化函数（加载图片和地图）
const initDataAndMap = async () => {
...
onMounted(async () => {
  ...
  if (planId) {
    message.loading({ content: '正在从云端提取行程...', key: 'loadPlan', duration: 0 })
    try {
      // 👉 修改：使用 apiBaseUrl
      const response = await fetch(`${apiBaseUrl}/api/trip/plan/${planId}`)
      const resData = await response.json()

      if (resData.success) {
        tripPlan.value = resData.data
        // 将拿到的数据存入缓存，方便用户刷新页面不丢失
        sessionStorage.setItem('tripPlan', JSON.stringify(tripPlan.value))
        message.success({ content: '行程提取成功！', key: 'loadPlan' })

        await initDataAndMap() // 初始化地图和图片
      } else {
        message.error({ content: resData.message || '行程提取失败', key: 'loadPlan' })
      }
    } catch (error) {
      console.error('拉取行程失败:', error)
      message.error({ content: '网络错误，无法加载行程单', key: 'loadPlan' })
    }
  } else {
    // 💻 场景 B：正常在网页端点击“开始规划”跳过来的，直接读本地缓存
    const localData = sessionStorage.getItem('tripPlan')
    if (localData) {
      tripPlan.value = JSON.parse(localData)
      await initDataAndMap() // 初始化地图和图片
    } else {
      // 既没有 ID，本地也没缓存，说明是直接敲网址进来的，踢回首页
      message.warning('未找到行程记录，请先创建')
      router.push('/')
    }
  }
})
